...
The second tension arises from the interplay of form and meaning. Chomsky's famous example (\ref{ex:colorless-grammatica}) demonstrated that syntax operates somewhat independently from semantics, proving that meaningless sentences can be structurally viable. But the converse doesn't hold: meaning often dictates grammaticality. When speakers reject (\ref{ex:tense}) \textit{*I've finished it yesterday}, they aren't identifying a structural crash (subject-verb-object-adverb is structurally sound), but a specific clash between the present perfect's meaning and the adverb's meaning.
...
This distinction parallels the measurement problem in other sciences: we can't directly observe temperature but have to infer it through the behaviour of thermometers. Similarly, grammaticality itself remains a theoretical construct that we access through various behavioural measures, with acceptability ratings being just one imperfect window into the underlying phenomenon.
...
\end{cases}
\end{equation*}
$\mathsf{map}$ is explicitly pre-grammatical: it filters out inputs that defeat structural analysis (e.g.\ word salad, category errors) but doesn't enforce community licensing. Community non-licensing belongs only in $C_t$.
