...
This utterance is structurally viable and semantically transparent (we understand the intended meaning immediately), but it is categorically rejected in English. The same form is grammatical in French (\textit{J'ai 25 ans}) and Spanish (\textit{Tengo 25 años}), demonstrating that the constraint is community-specific. Usage-based approaches \autocite{bybee2006} emphasize that such rejection arises from the absence of \textbf{community licensing} ($C_t$). The English-speaking community has conventionalized a different way to express age, effectively blocking this form.
