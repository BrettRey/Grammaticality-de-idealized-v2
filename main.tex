...
\ex\label{ex:colorless-grammatica} \textit{~Colorless green ideas sleep furiously.} \autocite{chomsky1957}
...
\begin{enumerate}
\item At its core, grammaticality involves conventional form--value relations within specific language communities, dialects, registers, and situations.
...
Locality influences both the subjective feeling of ungrammaticality and the licensing dynamics. In the feeling model (\S\ref{sec:feeling-new}), it appears as a component of the processing vector $\mathbf{P}_i$. In the dynamics, locality can be included among the utility features $\mathbf{f}(v;n,c)$ that determine $\rho_t(v\mid n,c)$ (\S\ref{sec:usage}). This lowers the counterfactual choice probability of high-locality variants, which both reduces their expected positive evidence stream $s_t$ and weakens the evidential force of their non-occurrence, since $p_t(u,c)$ scales with $\rho_t(u\mid n(u),c)$.
...
\textsc{Dependency locality}      & $L(u)$ (part of processing vector $\mathbf{P}_i$)\\
\textsc{Processing constraints}   & $\mathbf{P}_i(u,c)$ (interference, locality, surprisal)\\
...
\begin{table}[h]
\centering
\caption{Core notation and stability variables}
\label{tab:vars}
\begin{tabular}{@{}ll@{}}
\toprule
Symbol & Meaning \\
\midrule
$\mathbb{I}[\varphi]$ & Indicator function (1 if $\varphi$ is true, 0 otherwise) \\
...
$\mathsf{map}(u,c)$ & Mapping viability (1 if $M \to \mu$ is licensed in $c$) \\
...
$\widetilde{G}_t(u,c)$ & Graded stability score: $\mathsf{map} \cdot C_t \cdot K$ \\
$G_t(u,c)$ & Categorical membership predicate (thresholded $\widetilde{G}_t$) \\
...
high $\mathbf{P}_i$ cost, parse recovered &
...
Let $\mathsf{map}(u,c)\in\{0,1\}$ indicate whether there exists, for $u$ in $c$,
a licensed morphosyntactic analysis that yields a morphosyntactic meaning:
\[
\mathsf{map}(u,c)=
\begin{cases}
1 & \text{if }\exists a\in\mathcal{A}(u,c)\text{ such that }\mu=\mu(a)\text{ is well-defined}\\
0 & \text{otherwise.}
\end{cases}
\]
This is the only truly categorical failure mode: if no analysis is available,
there is no candidate form--value relation to stabilize.
...
$\Omega(u,c)$:
\[
p(\omega\mid u,c)\propto \exp\bigl(-E(\omega;u,c)\bigr),\qquad \omega\in\Omega(u,c).
\]
Define \emph{coherence} as the concentration of this distribution:
\[
K(u,c)=\max_{\omega\in\Omega(u,c)} p(\omega\mid u,c)\in(0,1].
\]
Intuitively, $K$ is high when a single construal dominates, and low when
construals are diffuse, mutually inconsistent, or repair-dependent.

Crucially, $E(\omega;u,c)$ is an \emph{open-ended} energy model:
\[
E(\omega;u,c)=\sum_{j\in\mathcal{J}(c)} w_j\,\phi_j(\omega;u,c),
\]
where $\mathcal{J}(c)$ is the set of coherence demands that are live in $c$
(temporal alignment, information-structural fit, indexical stance alignment, etc.).
The theory does not assume a fixed inventory of $\phi_j$; it assumes only that
coherence is measurable as concentration, and decomposable when needed.
...
-\mathbf{\gamma}^{\top}\mathbf{P}_{i}(u,c)
-\delta\,D_i(u,c)
+\varepsilon_i,
\]
...
$\mathbf{P}_{i}(u,c)$ is a vector of processing/repair costs (locality, interference,
...
\[
\rho_t(v\mid n,c)=\frac{\exp\bigl(U_t(v;n,c)\bigr)}{\sum_{v'\in\mathcal{V}_n}\exp\bigl(U_t(v';n,c)\bigr)},
\qquad
...
non-satiating rejection profile.  Additional processing penalties in $\mathbf{P}_i(u,c)$
...
incremental parsing. For clarity, we give the conditioning-known special case; the general form in \S\ref{sec:feeling-new} replaces $\widetilde{G}_t(u,c)$ with the hearer’s expected stability $\widetilde{G}^{(i)}_t(u\mid e)$ under uncertainty about $c$:

\[
F_{i,t}(u,c)=
 -\alpha\bigl(1-\widetilde{G}_t(u,c)\bigr)
 -\mathbf{\gamma}^{\top}\mathbf{P}_{i}(u,c)
 -\delta D_i(u,c)
 +\varepsilon_i ,
\]
...
\begin{itemize}
  \item $\widetilde{G}_t(u,c)\in[0,1]$ — graded stability score (§\ref{sec:formalism}).
  \item $\mathbf{P}_i(u,c)$ — vector of implementation costs (locality, interference,
        surprisal, garden-path reanalysis).
  \item $D_i(u,c)\in[0,1]$ — prescriptive dissonance for individual $i$.
  \item $\varepsilon_i \sim\mathcal N(0,\sigma^2)$ — speaker-specific bias.
\end{itemize}
...
\item In change-over-time studies, $F$ is the leading indicator: entrenchment
      first dampens cost terms in $\mathbf{P}_i$, then lifts $C_t$ and flips $G_t$ from $0$ to~$1$.
\end{enumerate}
...
Other cases that generative grammar struggles to explain are those that are grammatical with one meaning but ungrammatical with another, such as (\ref{ex:have-years-sem}) \textit{I have 16 years}. While syntactically identical to grammatical expressions like \textit{I have 16 dollars}, this construction becomes ungrammatical specifically when used to express age. A purely syntactic account must somehow explain why the same structure is well-formed in one case but ill-formed in another, despite no apparent syntactic differences. MVMG, in contrast, locates the source of ungrammaticality in the community's form--value conventions: \textit{have}+numeral years has become conventionalized for expressing duration or future time (\textit{I have 16 years until retirement}) but blocked for expressing age, where a different construction (\textit{I am 16 years old}) is the established pattern. Similar cases arise with plural forms that are grammatical with some meanings but not others (e.g., \textit{peoples} for ethnic groups but not multiple individuals) and with verbs that resist certain arguments despite no obvious syntactic prohibition (e.g., \textit{discuss about}). These meaning-dependent grammaticality patterns suggest that what gets blocked or licensed often depends on specific form--value associations rather than purely structural constraints.
...
odd‑sounding.  By contrast, \textit{doktor} in (\ref{ex:doktor}) keeps $K(u,c)=1$;
...
marked ``disharmonic''.  The community memorises them with $C_t(u,c)=1$, so no
