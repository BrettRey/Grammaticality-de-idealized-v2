...
These examples illustrate how each communicative situation defines its own grammaticality conditions. A form that is grammatical in one communicative situation may be ungrammatical when viewed from the perspective of another. The facts of grammaticality can also diverge for conversants who differ in situation, social position, or the norms they orient to. This divergence arises because grammaticality is communicative-situation-relative: the same utterance can be fully grammatical for those who share the relevant context and ungrammatical for those who don't. The linguist's task is therefore to determine whether a form is grammatical within any communicative situation, or systematically excluded across all.
...
Establishing a form--value mapping is a prerequisite for grammaticality, but it isn't sufficient. Individuals routinely interpret strings that are nevertheless judged ill-formed within the relevant communicative situation. That very flexibility forces the grammar to police \emph{how} a relation is licensed, not merely \emph{whether} one can be imagined in principle. In other words, the system needs additional filters that (i) rule out mappings that conflict with conventional constructional meanings (\S\ref{sec:semantic-compatibility}), (ii) flag mappings whose recovery cost overwhelms processing resources (\S\ref{sec:processing-constraints}), and (iii) block mappings that have never been conventionalized (\S\ref{sec:community-acceptance}). Only by layering these constraints on top of the initial relation does the framework predict the empirical fact that *\textit{Furiously sleep ideas green colorless} elicits universal rejection while (\ref{ex:center}) feels bad in everyday quotation yet is accepted as \enquote{grammatical but hard to process} once speakers are walked through the intended structure. The extra layers therefore partition the wide space of imaginable relations into the much smaller subset recognized as bona fide grammatical resources.
...
Situational entrenchment operates independently from the other components. A construction may have perfect semantic compatibility and minimal processing demands but still be rejected because a different form has been conventionalized for that meaning. This is particularly evident in cases of extreme rarity. Some constructions are so infrequent that speakers lack a shared consensus about their status. 
...
Even transparent, processable constructions fail when they violate established conventions. The \mention{sheeps} example shows how regularization can be blocked by entrenched irregular forms, while the extreme rarity of independent relative \mention{whose} prevents stable conventionalization.
...
Minor violations might involve a single component with partial conflict~-- for instance, a construction with moderate processing difficulty but full semantic compatibility and strong situational entrenchment. Such cases often receive intermediate acceptability ratings and may improve with exposure.
...
\item \textbf{Semantic--pragmatic coherence}: This factor, including information structure clashes, also constitutes part of $G_t(u,c)$. Semantic incompatibilities make utterances objectively ungrammatical within the relevant conventions.
