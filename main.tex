...
This utterance is likely structurally viable and semantically coherent (the meaning is obvious), but it is categorically rejected in English. Usage-based approaches \autocite{bybee2006} emphasize that such rejection arises from the absence of \textbf{community licensing} ($C_t$). The community has settled on a different way to express this meaning, effectively blocking this form.

\bigskip
These three problems -- structural viability, interpretive coherence, and community licensing -- correspond exactly to the three constitutive variables of the MVMG framework presented in Section \ref{sec:framework}. By acknowledging them as distinct but interacting components, we can move beyond the impasse of trying to force all grammaticality judgments into a single \enquote{competence} definition or a purely probabilistic usage model.

\section{The morphosyntactic-value model of grammaticality}\label{sec:framework}

This section presents the proposed MVMG framework. The term \emph{value} is used here in the Saussurean sense of \mention{valeur} \autocite{saussure1916}: the identity of a linguistic unit is constituted not by a direct label but by its systemic relations -- what it contrasts with, what contexts license it, what interpretations it makes available.
...
Here we find a clash in information structure: the same participant is simultaneously focused through fronting \mention{who} while being backgrounded by the relative clause construction \autocite{cuneogoldberg2023}.
...
\end{cases}
\end{equation*}
$\mathsf{map}$ is explicitly pre-grammatical: it filters out inputs that defeat structural analysis (e.g.\ word salad, category errors) but does not enforce community licensing. Community non-licensing belongs only in $C_t$.
...
\textsc{(iii) Community licensing (entrenchment) under conditioning.}
...
\begin{enumerate}
   ...
   $\mathsf{map}=1$, $K \approx 1$ (perfectly interpretable), but $C_t$ is low/indeterminate because the opportunity set $N_t$ is small (low preemption mass $p_t$). The Beta update has not driven $C_t$ to 0, leaving it in a state of \enquote{not established}. 
