...
Integration cost rises with dependency length; self-paced reading shows difficulty peaks at loci of long-distance integration, with locality accounting for 59--73\% of variance in reading times \autocite{GrodnerGibson2005}. I therefore model a dedicated locality cost $L(u)$, summing over the dependencies of $u$, rather than hiding the effect inside a residual term. The cost grows with dependency length but saturates at extreme distances (the precise functional form appears in \S\ref{sec:formalism}).
...
This mismatch between predicted and observed frequency leads to divergent speaker responses: some can make the analogical leap and assume the construction represents a legitimate, if rare, pattern; others make the leap but interpret its rarity as evidence that the construction isn't conventionally accepted; still others can't construct the analogy and either conclude the form is ungrammatical or question their own ability to process it correctly.
