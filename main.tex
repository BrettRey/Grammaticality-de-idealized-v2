...
At its foundation, grammaticality depends on stable pairings between morphosyntactic forms and their values within specific language communities. That such pairings exist at all levels of grammatical organization~-- from morphemes to abstract syntactic patterns~-- is a core insight of Construction Grammar \autocite{goldberg1995constructions}. Since language serves communication, meaning is typically the most salient aspect of a form's value, but value extends beyond semantics to include phonological and distributional regularities as well; the morphosyntax--phonology interface in Turkish vowel harmony provides a clear example (Appendix~\ref{app:turkish-harmony}).

Morphosyntactic forms, like words, are typically polysemous: a single form participates in multiple form--value pairings, usually with one dominant \autocite{Kilgarriff2004}. The English past tense, for instance, usually denotes past time, but it can also mark social distance (\mention{Could you help me?}) or low likelihood (\mention{If I went tomorrow\dots}). What matters for grammaticality is whether a stable pairing exists within the relevant community, not whether the form has a single fixed meaning.
