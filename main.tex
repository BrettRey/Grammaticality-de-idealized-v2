...
\fancyhead[RO]{\small\scshape Grammaticality de-idealized}
...
\begin{abstract}
    This paper introduces a novel theoretical framework for (un)gram\-maticality, distinct from traditional grammars focused solely on language production or description. The proposed model conceptualizes (un)grammaticality explicitly as coherence in conditioned form--value relations accepted or rejected by language communities, integrating morphosyntactic, semantic--pragmatic, and social dimensions. Previous approaches, including generative grammar, Construction Grammar, and psycholinguistic models, have not adequately explained why grammaticality is specifically restricted to morphosyntax, why semantically coherent constructions may nonetheless be deemed ungrammatical, or why certain semantically transparent structures remain systematically blocked. The present framework addresses these gaps by providing a principled explanation of (un)grammaticality rooted in community standards for communicative coherence, rather than relying exclusively on structural or intuition-based definitions.
\end{abstract}
...
Every competent speaker of English knows that \ungram{\mention{Can the have running}} is impossible, but the source of this certainty proves remarkably elusive. What do we mean when we say a sentence is ungrammatical? Consider these examples:
...
\item Grammaticality reflects the degree to which an utterance establishes stable, community-recognized form--value relations. Various factors can disrupt this stability---from complete absence of a licensable analysis to conflicts between competing construals to lack of community licensing---producing a spectrum of ungrammaticality effects unified by form--value instability.
\end{enumerate}
...
The concept of grammaticality remains elusive despite its centrality to linguistic theory. After decades of research, we still lack a comprehensive account of what makes an utterance grammatical or ungrammatical. This theoretical impasse stems from trying to treat grammaticality as a unified phenomenon, when it actually arises from the interaction of three distinct sources of failure. To understand why previous approaches have struggled, we must disentangle these three components: structural viability, interpretive coherence, and community licensing.

\subsection{The problem of structural viability}\label{sec:impasse-viability}

The first challenge in defining grammaticality is distinguishing between structures that fail because they are fundamentally structurally unsound and those that fail for other reasons. \textcite{chomsky1957}, building on formal production systems developed by \textcite{post1943}, treated grammaticality as a categorical property defined by membership in a set of well-formed strings. This approach captures the intuition that some strings, like (\ref{ex:nonsense}), are essentially non-computable by the parser:

\ea
\ungram{\mention{Can the have running?}}
\z

Here, the input crashes the structural analysis independent of meaning. The "categorical" view correctly identifies that without a viable structural analysis (what conceptualize as $\mathsf{map}\neq 0$), an utterance cannot be grammatical. However, by elevating this single condition to the *definition* of grammaticality, formal approaches struggled with evidence that speakers consistently provide gradient judgments for structures that are clearly parseable but degraded. The competence-performance distinction \autocite{chomsky1965} attempted to preserve categorical grammar by attributing gradience to processing limitations. Yet as \textcite[71]{schutze2016} notes, this often leads to circularity, where problematic results are dismissed as performance artifacts. The MVMG framework resolves this recognizing that while structural viability (the successful mapping of form to category) is a necessary, categorical prerequisite, it is not the sole determinant of grammaticality.

\subsection{The problem of interpretive coherence}\label{sec:impasse-coherence}

The second tension arises from the interplay of form and meaning. Chomsky's famous example (\ref{ex:colorless-grammatica}) demonstrated that syntax operates somewhat independently from semantics, proving that meaningless sentences can be structurally viable. But the converse does not hold: meaning often dictates grammaticality. When speakers reject (\ref{ex:tense}) \textit{*I've finished it yesterday}, they are not identifying a structural crash (subject-verb-object-adverb is structurally sound), but a specific clash between the present perfect's meaning and the adverb's meaning.

Generative semanticists like \textcite{lakoff1971} and \textcite{mccawley1968} demonstrated early on that many constraints have semantic motivations. Construction Grammar \autocite{goldberg1995constructions} has further shown how constructions carry meanings that interact with lexical semantics. Novel uses that align with established constructional meanings (like \mention{She texted him the address}) are accepted, while those that clash (like \textit{*She disappeared him the evidence}) are rejected. A complete theory must therefore account for **interpretive coherence** ($K$): the degree to which a structurally viable form yields a stable, non-contradictory interpretation.

\subsection{The problem of community licensing}\label{sec:impasse-licensing}
...
Consider left-branch extraction violations like (\ref{ex:lbe}):

\ea
\ungram{\mention{Which did you buy car?}}
\z

This utterance is likely structurally viable (in terms of raw parsing) and semantically coherent (the meaning is obvious), yet it is categorically rejected in English. Usage-based approaches \autocite{bybee2006} emphasize that such rejection arises from the absence of **community licensing** ($C_t$). The community has settled on a different way to express this meaning, effectively blocking this form.

\bigskip
These three problems -- structural viability, interpretive coherence, and community licensing -- correspond exactly to the three constitutive variables of the MVMG framework presented in Section \ref{sec:framework}. By acknowledging them as distinct but interacting components, we can move beyond the impasse of trying to force all grammaticality judgments into a single "competence" definition or a purely probabilistic usage model.

\section{The morphosyntactic-value model of grammaticality}\label{sec:framework}

This section presents the proposed MVMG framework. The term \emph{value} is used here in the Saussurean sense of \textit{valeur} \autocite{saussure1916}: the identity of a linguistic unit is constituted not by a direct label but by its systemic relations -- what it contrasts with, what contexts license it, what interpretations it makes available.
Value here means Saussurean \mention{valeur} (systemic contrastive identity), not feature-value assignment or decision-theoretic value.
Throughout, I treat grammatical knowledge as a conditioned \textbf{form--value relation}; when this relation is sufficiently stable in a community -- i.e.\\ when a dominant value is reliably recoverable and socially licensed -- I refer to it informally as a \textbf{form--value pairing}.

The formal architecture rests on three constitutive variables -- morphosyntactic mapping ($\mathsf{map}$), interpretive coherence ($K$), and community licensing ($C_t$) -- whose interaction determines grammaticality (\S\ref{sec:formalism}). For expository purposes, however, it is useful to distinguish recurrent \emph{instability modes}: diagnostic categories that correspond to different ways of driving $\mathsf{map}$, $K$, or $C_t$ toward zero, plus processing and prescriptive factors that modulate the subjective feeling $F_{i,t}$ without affecting grammatical status directly.

\subsection{Diagnostic categories}\label{sec:core-components}

The diagnostic categories described below are not primitives of the model but rather convenient labels for the most common routes to ungrammaticality. Each can be traced to one or more of the constitutive variables, as the formal core in \S\ref{sec:formalism} makes explicit.
...
\paragraph{Form--value relations}
...
Throughout, we treat the discrete Bayesian update in \S\ref{sec:update} as the primary dynamics module; the continuous-time logistic form $\dot{C}_t = \Delta C_t(1-C_t)$ is demoted to a mean-field approximation for qualitative analysis of the expected trajectory.
...
\textsc{Community licensing}      & $C_t(u,c) \in [0,1]$ (entrenchment dynamic)\\
...
$C_t(u,c)$ & Community Licensing (Social convention) \\
\bottomrule
\end{tabular}
...
\midrule
$\mathsf{map}=0$ &
\emph{nonsense} (\ungram{\mention{Can the have running}})\\[4pt]
$K(u,c)\approx 0$ &
...
high $\mathbf{R}_i$ cost, parse recovered &
...
\paragraph{No viable form--value relation}
...
\paragraph{Processing overload}\label{sec:processing}
...
\paragraph{Apparent categorical gaps}\label{sec:systematic-blocking}
Some constructions are rejected so persistently that they behave as if categorically banned. Under MVMG, these are analysed as \emph{stable gaps}: $\mathsf{map}(u,c)=1$ and $K(u,c)$ can be high, but community licensing is driven toward $C_t(u,c)\approx 0$ by persistent preemption in a large opportunity set (\S\ref{sec:update}, \S\ref{sec:categorical-without-bans}). Left-branch extraction is the textbook case.
...
The feeling of ungrammaticality can be conceptualized as a negative response triggered by the detection of unstable or missing form--value relations. This response wouldn't define grammaticality itself but would instead serve as a speaker's heuristic detection mechanism. (For the sake of this conceptual overview, we assume the speaker treats the conditioning state $c$ as known; the full formal definition in \S\ref{sec:feeling-new} generalizes this to handle uncertainty about context.) This distinction helps explain:
...
\item \textbf{Sociolinguistic appropriateness}: This factor shows mixed status. Clear indexical clashes can affect $G_t(u,c)$ when they prevent the intended social meaning from being conveyed. However, mere stylistic infelicities typically affect only $F_{i,t}$.
...
(i) a \emph{state theory} specifying what it is for an utterance type to be grammatical
\emph{for a population in a communicative situation} at time $t$;
(ii) a \emph{dynamics module} specifying how such states tend to arise and persist.
The state theory is constitutive; the dynamics module is etiological.

\subsection{Conditioning structure}
\label{sec:conditioning}

Let $c\in\mathcal{C}$ be a \emph{conditioning state} -- a construed communicative situation
...
Let $u$ range over utterance \emph{types} (constructional tokens abstracted over),
...
\paragraph{(i) Mapping viability}
Let $\mathsf{map}(u,c)\in\{0,1\}$ indicate whether the parser can construct at least one well-typed morphosyntactic representation for $u$ (category assignment + structural composition) throughout the derivation.
\begin{equation*}
\mathsf{map}(u,c)=
\begin{cases}
1 & \text{if }\exists \text{ valid parse } M(u) \text{ yielding } \mu(M)\\
0 & \text{otherwise.}
\end{cases}
\end{equation*}
$\mathsf{map}$ is explicitly pre-grammatical: it filters out inputs that crash the parser (e.g.\ word salad, category errors) but does not enforce community licensing. Community non-licensing belongs only in $C_t$.

\paragraph{(ii) Coherence as concentration of interpretation}
...
$$
We use the maximum probability rather than entropy to directly represent the availability of a single dominant construal. While entropy would capture similar information about distributional peakedness, the maximum probability formulation aligns more naturally with the winner-take-all nature of conscious interpretation in online processing.

Crucially, $E(\omega;u,c)$ is an \emph{open-ended} energy model:
\begin{equation*}
E(\omega;u,c)=\sum_{j\in\mathcal{J}(c)} w_j\,\phi_j(\omega;u,c),
\end{equation*}
where $\mathcal{J}(c)$ can include any constraint live in $c$. In the demonstrations below, we assume a minimal fixed inventory of $\phi_j$: (1) temporal alignment (e.g.\ tense vs. adverbials), (2) argument structure satisfaction, (3) information-structural fit (topic/focus alignment), and (4) indexical consistency. The framework remains open to others, but these four suffice to derive the main diagnostic categories.

This decomposition allows us to bridge the narrative diagnostic categories of \S\ref{sec:core-components} to the formal variables. For instance, we may group coherence demands by their subsystem -- denoting the sum over purely semantic demands as $E_{\text{sem}}$ and over indexical/pragmatic demands as $E_{\text{index}}$ -- to yield an expositional factorization $K \approx K_{\text{sem}} \cdot K_{\text{index}}$. In practice, however, the framework assumes that stability depends on the joint concentration of all constraints live in the communicative situation $c$.

\paragraph{(iii) Community licensing (entrenchment) under conditioning}
...
either a pairing counts as an available resource in $c$ or it does not.
...
\end{equation*}
The threshold $\tau(c)$ is not a hidden grammatical parameter. It is a decision
...
item as \textsc{in-repertoire} when it is not, and $L_{\textsc{fr}}(c)$ the loss
...
\subsection{What $\mathsf{map}$ does and does not do}
...
bans} are not treated here as mapping failures. In those cases, the intended
...
\paragraph{Interpretation:} $F=0$ corresponds to a null affect (no anomaly reported), while $F<0$ is a metacognitive warning proportional to the summed evidence of misfiring. The same feeling can arise from different mixtures of causes (structural, procedural, or social).

\paragraph{Measurement strategy:} $F$ can be measured via explicit ratings (mapped linearly to $[-1,0]$) or implicit probes like N400/P600 magnitudes and self-paced reading slowdowns (proxies for components of $\mathbf{R}_i$).
...
We can now define the status of the four canonical examples (\S\ref{sec:diag-tree}) by computing their component values.

\begin{enumerate}
   \item \textbf{\ungram{\mention{Can the have running}}} (Nonsense): 
   $\mathsf{map}=0$ because the parser fails to assign a valid category structure. 
   ...
   \item \textbf{?friend of whose} (Community-Novel): 
   $\mathsf{map}=1$, $K \approx 1$ (perfectly interpretable), but $C_t$ is low/indeterminate because the opportunity set $N_t$ is small (low preemption mass $p_t$). The Beta update has not driven $C_t$ to 0, leaving it in a state of ``not established''. 
...
\end{axis}
\end{tikzpicture}
\caption{Simulation of community licensing rate $C_t$ under preemption. When effective opportunity mass is high (Stable Gap, red), $C_t$ is driven rapidly to zero. When opportunities are rare (Community-Novel, blue), $C_t$ declines slowly, leaving the construction in a state of indeterminacy rather than categorical rejection.}
...
refinements of the conditioning partition itself).  It does not define grammaticality.

\subsection{Niches, competitors, and opportunity sets}
\label{sec:niches}

Let $n$ index a \emph{constructional niche} (a communicative job), and let
...
\subsection{Usage as licensing $\times$ choice among licensed options}
\label{sec:usage}

Separate \emph{licensing} from \emph{selection}.  A variant can be licensed but rarely chosen.
...
not chosen. The update should not treat all \enquote{opportunities} as equally
...
For a target variant $u$ associated with niche $n(u)$, define the \emph{effective
...
Loanwords such as \mention{doktor} `doctor' violate backness harmony, yet are fully acceptable;
