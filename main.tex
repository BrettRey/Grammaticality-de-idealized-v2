...
$C_t(u,c)\approx 0$, rarely encountered &
community-novel (\textit{friend of whose})\\[4pt]
$C_t(u,c)\approx 0$, strongly preempted &
...
\item \textbf{Value coherence}: This factor, including information structure clashes, also constitutes part of $G_t(u,c)$. Incompatibilities among the values encoded by form--value relations make utterances objectively ungrammatical within the relevant conventions.
\item \textbf{Situational entrenchment/frequency}: High-frequency patterns that speakers have conventionalized represent part of $G_t(u,c)$. A pattern that has never been established simply isn't grammatical, regardless of its theoretical possibility.
\item \textbf{Sociolinguistic appropriateness}: This factor shows mixed status. Clear indexical clashes can affect $G_t(u,c)$ when they prevent the intended social value from being conveyed, but mere stylistic infelicities typically affect only $F_{i,t}$.
