...
\begin{enumerate}
    \item conventional morphosyntactic--meaning pairings;  
    \item compatibility between those meanings and contextual meaning;  
    \item incremental‑processing limits;  
    \item degree of community entrenchment;  
    \item near-zero-entrenchment patterns (sometimes described as structural bans).
\end{enumerate}
...
\enquote{what we experience is a kind of inference: it’s the brain’s best guess about what’s going on in some way out there in the world.
...
\begin{enumerate}
    ...
    \item \textbf{Near-zero entrenchment (structural bans)}~-- structural patterns whose community entrenchment has settled near zero.
\end{enumerate}
...
\ea[?]{\textit{I saw Joan, a friend of whose was visiting.}}\label{ex:whose}
\z
...
\begin{tcolorbox}[colback=lsLightBlue!30]
\textbf{Diagnostic}: Does the construction instantiate a configuration whose entrenchment is effectively zero in the speech community?
...
\node[decision, below=of map]  (ban)  {near-zero entrenchment?};
\node[outcome , right=of ban]  (lbe)  {near-zero entrenchment\\(*\textit{Which did you buy \_ car?})};
...
\paragraph{Innovation cues.}
A subset of constructions enters the speech community through what \textcite{Bergs2025} calls \textsc{extravagance}~-- the speaker’s calculated push for noticeability~-- and through transient spikes in \textsc{sociocognitive salience}.  In MMMG these cues are treated as initial conditions on the entrenchment trajectory \(C^{t}(u)\), not as separate predictive terms.  They raise the starting point of the logistic curve but require no additional machinery.
...
But NOT for:
\begin{itemize}
\item Near-zero-entrenchment violations (determiner extraction) (available data)
...
MMMG retains the insight that certain configurations are systematically excluded, but explains these \enquote{near-zero entrenchment} (structural bans) as the outcome of extreme, community‑level stability rather than as inviolable innate rules.
