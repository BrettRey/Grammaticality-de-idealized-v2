...
\begin{abstract}
    This paper introduces a novel theoretical framework for (un)gram\-maticality, distinct from traditional grammars focused solely on language production or description. The proposed model conceptualizes (un)grammaticality explicitly as coherence in conditioned form--value relations accepted or rejected within communicative situations, integrating morphosyntactic, semantic--pragmatic, and social dimensions. Previous approaches, including generative grammar, Construction Grammar, and psycholinguistic models, haven't adequately explained why grammaticality is specifically restricted to morphosyntax, why semantically coherent constructions may nonetheless be deemed ungrammatical, or why certain semantically transparent structures remain systematically unentrenched. The present framework addresses these gaps by providing a principled explanation of (un)grammaticality rooted in conventionalized norms within communicative situations, rather than relying exclusively on structural or intuition-based definitions.
\end{abstract}
...
\subsubsection{Morphosyntactic form--value relations within communicative situations}\label{sec:f-m-pair-in-community}

At its foundation, grammaticality depends on stable relationships between morphosyntactic forms and their values within specific communicative situations. That such relationships exist at all levels of grammatical organization~-- from morphemes to abstract syntactic patterns~-- is a core insight of Construction Grammar \autocite{goldberg1995constructions}. Since language serves communication, meaning is typically the most salient aspect of a form's value, but value extends beyond semantics to include phonological and distributional regularities as well; the morphosyntax--phonology interface in Turkish vowel harmony provides a clear example (Appendix~\ref{app:turkish-harmony}).

Morphosyntactic forms, like words, are typically polysemous: a single form participates in multiple form--value relationships, usually with one dominant \autocite{Kilgarriff2004}. The English past tense, for instance, usually denotes past time, but it can also mark social distance (\mention{Could you help me?}) or low likelihood (\mention{If I went tomorrow\dots}). What matters for grammaticality is whether a stable relationship exists within the relevant communicative situation, not whether the form has a single fixed meaning.
...
Following \textcite[3]{wiese2023}, grammaticality emerges from regularities within a \enquote{communicative situation}.

Communicative situations aren't static or mutually exclusive. A speaker navigates overlapping contexts that shift on multiple dimensions: the immediate situation (formal meeting vs.\ casual chat), durable social positions (regional, generational), and the norms they're orienting to (which can shift with audience). This fluidity explains style-shifting, dialect emergence, and the gradual acceptance of initially marginal constructions. What matters for grammaticality is the stability of form--value relationships within the relevant communicative situation.
...
\subsubsection{Value compatibility}\label{sec:value-compatibility}

Beyond the existence of form--value relations, grammaticality requires compatibility among the values those relations encode. This component captures cases where individual elements are well-formed but their combination creates conflicts in the composite value~-- whether semantic, pragmatic, or information-structural.

Consider the temporal incompatibility in:
\ea[*]{\mention{I've finished it yesterday.}}\label{ex:tense-sem}
\z
Here, the present perfect construction encodes current relevance while \mention{yesterday} specifies completed past time. Unlike lexically incongruous combinations like \mention{colorless green ideas}, which remain grammatical because the clash doesn't involve morphosyntactic form--value relations, this example shows a direct conflict between the temporal value encoded by the grammatical construction and the lexical temporal specification.

Another type of value incompatibility arises when a construction is used with a meaning it can't encode:
\ea[*]{\textit{I have 25 years.} \hfill intended as `I'm 25 years old'}\label{ex:have-years-sem}
\z
In English, \mention{have}+\mention{years} denotes relational predication between agents and temporal intervals (like periods until retirement or spans of experience), rather than ascribing a temporal measure of age. This value mismatch makes the construction inappropriate for expressing age.
...
These examples illustrate how value compatibility operates as an independent component of grammaticality. Even when morphosyntactic forms exist and are properly combined according to structural rules, the values they encode need to align coherently for the construction to be grammatical.
...
When the values encoded by morphosyntactic and lexical elements clash, the construction fails despite having well-formed individual components. This includes temporal conflicts (as in \ungram{\mention{I've finished it yesterday}}), inappropriate predication types (as in \ungram{\mention{I have 25 years}} for age), and information-structural contradictions (as in the lifeguard example).
...
Even transparent, processable constructions fail when they violate established conventions. The \mention{sheeps} example shows how regularization can be preempted by entrenched irregular forms, while the extreme rarity of independent relative \mention{whose} prevents stable conventionalization.

\textsc{Apparent categorical gaps.}\label{sec:stable-gaps}
Some constructions are rejected so persistently that they behave as if categorically banned. Under MVMG, these are analysed as \term{stable gaps}: the structure is viable and interpretation coherent, but situational licensing is driven toward zero by persistent preemption in a large opportunity set (\S\ref{sec:update}, \S\ref{sec:categorical-without-bans}). Left-branch extraction is the textbook case.
...
\begin{itemize}
\item \textbf{Morphosyntactic licensing}: When this factor loads heavily on rejected items, it likely reflects a core component of objective grammaticality. Violations of morphosyntactic constraints directly prevent stable form--value mappings.
\item \textbf{Value coherence}: This factor, including information structure clashes, also constitutes part of objective grammaticality. Incompatibilities among the values encoded by form--value relations make utterances objectively ungrammatical within the relevant conventions.
\item \textbf{Situational entrenchment/frequency}: High-frequency patterns that speakers have conventionalized represent part of objective grammaticality. A pattern that has never been established simply isn't grammatical, regardless of its theoretical possibility.
\item \textbf{Sociolinguistic appropriateness}: This factor shows mixed status. Clear indexical clashes can affect objective grammaticality when they prevent the intended social value from being conveyed, but mere stylistic infelicities typically affect only the subjective feeling.
\item \textbf{Processing cost/memory load}: This factor clearly belongs to the subjective feeling rather than objective grammaticality. Heavy processing demands can depress ratings even for constructions that violate no grammatical constraints.
\item \textbf{Morphophonological well-formedness}: When phonological patterns realize obligatory morphosyntactic features, violations affect objective grammaticality. Otherwise, they merely influence the subjective response.
\end{itemize}

This distinction between objective grammaticality and subjective feeling has immediate methodological consequences. If research aims to uncover the grammar itself, investigators should model only those factors that map onto objective grammaticality while treating others as measurement noise or scale covariates. Conversely, research into human judgment behaviour should retain the full factor structure, as it is precisely the subjective feeling that predicts how speakers will rate sentences presented out of context.
...
$K(u,c)$ & Interpretive coherence ($\in[0,1]$)\\
...
$\rho_t(v\mid n,c)$ & Choice probability among licensed variants\\
...
\begin{enumerate}
   ...
   $\mathsf{map}=1$, $K \approx 1$, but $C_t \to 0$ with high certainty. Since $N_t$ is large (questions are frequent) and the counterfactual choice probability $\rho_t$ is high, the effective preemption mass $p_t$ is large. This drives the Beta posterior mass rapidly to zero (Figure~\ref{fig:simulation}).
...
Recent work connects CxG's framework to model-internal analysis. \textcite{weissweiler2023construction} demonstrate how construction grammar provides a theoretical framework for probing how neural language models handle different levels of linguistic abstraction. Their findings suggest that transformer models may learn construction-like representations, providing evidence about model-internal abstraction and tools for analyzing artificial neural networks.
