...
A useful analogy: operators function as protocol headers rather than payload content. They configure how an utterance is to be processed~-- what kind of move it is, who is committed to what, how subsequent contributions are constrained~-- rather than contributing the conceptual substance of what's being communicated. This framing helps distinguish three layers of linguistic infrastructure: expression-shape constraints that make an utterance recognisable as a token of the system (phonotactics, morphotactics, prosodic well-formedness); operator constraints that configure public update; and payload resources (open-class lexicon, indexical field) that are negotiable and extensible. The operator stratum is the middle layer. Categorical \enquote{not a word} reactions target the first; categorical \enquote{you can't say that (as a move)} reactions target the second; payload mismatches invite negotiation rather than rejection.
...
A second illustration: Japanese clause-typing particles. The sentence-final particle \mention{ka} marks a clause as interrogative; its absence (with declarative intonation) marks it as assertive. The particle doesn't contribute conceptual content; it configures what kind of move the utterance counts as and what responses are licensed. Omitting \mention{ka} in a context where a question is intended doesn't produce a \enquote{stylistic} or \enquote{informal} variant; it produces a declarative, changing the update type entirely. The interactional consequence is immediate: the addressee's uptake obligations shift. This is the sense in which clause-typing is an operator: the contrast is small, the selection is obligatory, and the consequences for public coordination are categorical.
...
\textsc{Phonological operators vs.\ accent.} The operator/indexical distinction cuts across the substance of the signal. Tonal languages provide a crucial test case. In many Bantu languages, specific tonal melodies mark tense, aspect, or polarity \citep{hyman2011}. These are operators, not \enquote{accent} features. For example, in Luganda and related languages, tense distinctions are realised primarily through tonal patterns on the verb: a high tone melody may mark remote past while a low tone melody marks recent past, with minimal or no segmental difference between the forms.
...
A common objection is that speakers frequently label accent or dialect choices as \enquote{bad grammar} to enforce social hierarchies. Acknowledging the operator stratum doesn't require denying this reality. Instead, it provides a way to distinguish first-order interactional crashes from second-order ideological policing. Overt correction is rare in either case, but the two types differ in what they target. An operator mismatch (a switch-reference error, a missing evidential) disrupts uptake: hearers may fail to track reference or may lack the epistemic anchoring the utterance presupposes. An accent or stigmatised variant that's clearly understood doesn't crash the update; the reaction targets the person's social index, not the utterance's structural function \citep{agha2007,eckert2012}. The fact that ideologies misappropriate the term \enquote{grammar} for the latter doesn't dissolve the reality of the former.
