...
- `rival-baselines.csv`: live stronger rivals that the AGR lane must beat, currently including a
  construction-specific listing baseline.
...
Rival-facing tranche:

- `low-frequency-qn-listing-rival`, a direct discriminator between `AGR` and the construction-
...
`rival-baselines.csv` records the stronger construction-specific listing rival. That rival can
accommodate already observed cells after their local distributions are known. The registered
low-frequency QN tranche is the next pressure point because it tests generalization before local
item preferences are established.

The first low-frequency-QN direct-string run returned no matches for all six registered cells
