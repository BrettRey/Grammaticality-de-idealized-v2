...
\begin{tikzpicture}[
  ...
  % === MODULE 1 (left side) ===
  \node[observed] (opp) at (-1.5,0) {\shortstack{opportunity\\set $N^{\ast}$}};
  \node[observed] (comp) at (-1.5,-1.5) {\shortstack{competitor\\tokens}};
  \node[latent] (ft) at (-1.5,-3) {$f_t$};
  \node[observed] (supply) at (1.5,0) {supply};

  % === CENTRAL: posterior, C_t, and constitutive ===
  \node[latent] (theta) at (3.5,0) {$\theta_u$};
  \node[latent] (Ct) at (3.5,-1.5) {$C_t$};
  \node[latent] (Gtilde) at (3.5,-3) {$\widetilde{G}_t$};
  \node[observed] (map) at (1.5,-2.4) {$\mathsf{map}$};
  \node[observed] (K) at (1.5,-3.4) {$K$};

  % === MODULE 2 (right side) ===
  \node[observed] (equil) at (5.5,0) {\shortstack{coordination\\equilibrium}};
  \node[observed] (dev) at (5.5,-1.5) {\shortstack{deviation\\penalties}};
  \node[observed] (expect) at (5.5,-3) {\shortstack{crystallized\\expectations}};
