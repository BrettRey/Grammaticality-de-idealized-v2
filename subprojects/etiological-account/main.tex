...
\end{tikzpicture}
\caption{The two-module architecture as a DAG. Module~1 (left): competitor tokens yield effective failures $f_t$ (= $N^{\ast}$), which update the posterior $\theta_u$; the latent supply mechanism generates observed tokens $s_t$ that provide positive evidence. Module~2 (right): the coordination regime (payoff structure, norm system) is exogenous~-- historically and institutionally set~-- and generates deviation penalties that shape crystallized expectations, which update $C$; the regime's maintenance is via this dev $\to$ expect $\to$ $C$ pathway across episodes. Processing costs (influenced by $C$) lower $\mathsf{map}$, implementing option~(i). Dotted arrows show constitutive relations: $C$, $\mathsf{map}$, and $K$ yield $\widetilde{G}$, which determines $F_u$ (label $t$ indicates synchronic). The feedback arrow shows $F_u$ triggering deviation penalties. The $t{\to}t{+}1$ label on the expectations $\to$ $C$ arrow indicates this effect operates across learning episodes.}
\label{fig:modules}
\end{figure}
