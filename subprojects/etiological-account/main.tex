...
The two modules can be read as complementary faces of one dynamic: learners infer repertoire status from distributional opportunity patterns, and communities maintain (and signal) repertoire boundaries through coordination pressures whose penalties are themselves publicly observable. Innovation~-- the entry of new forms into the repertoire~-- is not a third module but the positive-evidence channel that makes $s_t$ nonzero in the same evidential dynamics that underwrite preemption; whether that evidence is amplified into invasion is determined by the coordination equilibria, with drift as the finite-sampling noise in both directions.
