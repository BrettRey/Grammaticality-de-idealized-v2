...
\paragraph{Drift as baseline.} In finite populations, variant frequencies fluctuate even under unbiased transmission. Treat drift as the null model: preemption and coordination are departures from neutrality that respectively (i) bias inference under informative absence and (ii) introduce frequency-dependent reinforcement via deviation penalties. Empirically, the question is whether an observed exit or stability pattern exceeds what drift alone would predict given population size and transmission fidelity, and whether it tracks the distinct signatures of the two modules (opportunity-mass correlations for preemption; penalty and stake-sensitivity correlations for coordination). The \enquote{unstable} cell in the 2×2 typology (\S\ref{sec:integration}) is the drift-dominated region: neither bias is strong enough to override stochastic fluctuation.
