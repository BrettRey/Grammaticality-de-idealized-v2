...
The constitutive account decomposes grammatical status into three quantities:
\begin{enumerate}
  \item $\mathsf{map}(u,c)$: \term{mapping viability}. Does the form support a systematic pairing of structure and value? If parsing fails or no coherent structure-value mapping exists, $\mathsf{map}$ is zero.
  \item $K(u,c)$: \term{interpretive coherence}. Do the values carried by the form cohere with one another and with the conditioning state? Semantic anomaly, pragmatic incongruity, and logical incoherence lower $K$.
  \item $C_t(u,c)$: \term{repertoire status}. Is the form part of the community's grammatical repertoire for conditioning state $c$ at time $t$? This is the socially contingent component: it can vary across dialects, registers, and eras without any change in $\mathsf{map}$ or $K$.
\end{enumerate}

These combine multiplicatively into a stability score:
\begin{equation}\label{eq:stability}
\widetilde{G}_t(u,c) = \mathsf{map}(u,c) \cdot K(u,c) \cdot C_t(u,c).
\end{equation}
If any component is zero, the product is zero: mapping failure can't be rescued by repertoire status, and repertoire exclusion can't be rescued by perfect coherence.

Categorical membership is modeled by thresholding:
\begin{equation}\label{eq:membership}
G_t(u,c) = \mathbb{I}\!\left[\widetilde{G}_t(u,c) \ge \tau(c)\right],
\end{equation}
where $\tau(c)$ is a situation-sensitive threshold reflecting coordination stakes. High-stakes communicative situations raise $\tau$; informal, in-group contexts lower it.
...
\paragraph{What $C_t$ is.} $C_t(u,c)$ is a community-level variable: the probability that a randomly sampled competent community member will treat $u$ as licensed in conditioning state $c$, or equivalently, the equilibrium mass the community assigns to $u$ in that niche. Individual learners maintain beliefs about licensing~-- call these $\theta_{u,i}$ for speaker $i$~-- and $C_t$ is the population distribution of those beliefs together with the publicly enforced reaction norms that make those beliefs mutually visible. Module~1 below provides a microfoundation for how individual posteriors collapse toward zero under preemption. Module~2 provides a microfoundation for how reaction norms (correction, repair, social penalty) make those posteriors stable and mutually reinforcing. The two modules target different levels of analysis but converge on the same community-level outcome.

\paragraph{The role of the feeling of ungrammaticality.} Both modules operate through their effects on the \term{feeling of ungrammaticality} $F$~-- the subjective signal that a form is anomalous. Preemption accumulates because forms lacking positive evidence feel increasingly odd; the negative affect associated with producing or encountering them drives avoidance, which feeds back into the opportunity sets. Coordination equilibria are maintained because deviation triggers negative affect that prompts correction, repair initiation, or social sanction. $F$ is the proximate mechanism; $C_t$ is the state it tracks imperfectly. The constitutive account correctly insists that $F \ne \widetilde{G}$ (the feeling isn't identical to the objective status), but the etiological account shows that $F$ is nonetheless central to the causal pathway: it's the signal through which both learning and coordination dynamics operate.
...
\begin{itemize}
  \item \textbf{High-opportunity niche:} The form faces thousands of competitor tokens per year. $p_t$ is massive, the posterior crashes to near-zero early in acquisition, and $C_t$ stabilizes near zero. This is the profile of categorical exclusion.
...
\paragraph{Drift.} Neither preemption nor coordination captures \term{drift}: stochastic fluctuation in finite populations that can move $C_t$ in either direction independently of systematic pressures. Low-frequency forms are drift-vulnerable: they can exit the repertoire by chance even without active preemption, simply through bad luck in a finite population~-- no tokens in a generation, followed by learners who infer exclusion from the silence.

Drift matters for the stability typology. A form might be:
\begin{itemize}
  \item \textbf{Preemption-vulnerable:} strong competitor, high opportunity mass~-- actively pushed out.
  \item \textbf{Drift-vulnerable:} no strong competitor, but low frequency~-- stochastically wandering, could exit by chance.
  \item \textbf{Coordination-protected:} high frequency, mutual expectations~-- resists both preemption and drift.
\end{itemize}

The \enquote{unstable} cell in the 2×2 typology (\S\ref{sec:integration}) isn't just \enquote{weakly sustained}~-- it's drift-vulnerable. And the prediction for entry: drift can occasionally push a rare form above threshold, where coordination dynamics can then lock it in.
...
\begin{itemize}
  \item \textbf{Preemption strength:} Effective opportunity mass $p_t$ (possibly weighted), or the posterior mean/credible interval of $\theta_u$ under a specified prior. High $p_t$ with narrow credible interval around zero indicates strong preemption.
...
\emph{Disconfirmation:} If register/genre manipulation doesn't differentially affect preemption-dominated vs.\ doubly sustained gaps, then either the profile categorization is wrong or the modules aren't independent.
...
\emph{Disconfirmation:} If prescriptive norms are distributed uniformly across stratum levels~-- if payload mismatches attract as much institutional policing as operator violations~-- then the operator/payload distinction doesn't track coordination cost as predicted.
...
Third, the framework explains stable variation~-- competing forms that persist indefinitely~-- through domain-specific equilibria. When different conditioning states (registers, communities, interactional frames) occupy different coordination equilibria, coexistence is stable rather than anomalous.

Fourth, the framework connects grammatical structure to broader theories of convention, norm enforcement, and cultural evolution. The resources deployed here~-- Lewis on coordination, O'Connor on conventions, Richerson and Boyd on moralistic punishment, Young on stochastic stability~-- are domain-general. The etiological account positions grammar within the broader ecology of socially sustained regularities, where normed systems produce categorical boundaries through ongoing dynamics.
