...
What explains these differences? Knowing what grammaticality \emph{is}~-- what makes a form grammatical or not~-- doesn't automatically answer this question. Even if we grant the view that grammaticality depends partly on whether a form is in the community's repertoire, we still need to explain \emph{why} some forms get excluded from that repertoire and stay excluded, while others drift in or out over time.
...
\begin{enumerate}
  \item \textbf{Opportunity-sensitive learning.} Learners update the status of a form based on its success rate relative to effective opportunities. Positive evidence (hearing the form used) drives status up, entrenching a construction; systematic absence in high-opportunity niches drives it down (\term{preemption}). The evidential strength of absence depends on the size of the opportunity set: zero tokens out of five opportunities tells us little; zero tokens out of five thousand is decisive.
...
The two modules are complementary, not competing. Opportunity-sensitive learning explains how repertoire status gets updated~-- entrenched by positive evidence, excluded by preemption; coordination explains why established patterns \emph{persist}. A form can be excluded by strong preemption alone, by strong coordination alone, or by both. The combination determines the stability profile: gaps sustained by both modules are maximally stable; gaps sustained by only one module drift under the right conditions; gaps sustained by neither never fully categoricalize.

The two modules can be read as analytically separable mechanisms in one coupled system: learners infer repertoire status from distributional opportunity patterns, and communities maintain (and signal) repertoire boundaries through coordination pressures whose penalties are themselves publicly observable. Supply~-- the injection of tokens for forms not yet mutually expected~-- constitutes the positive-evidence channel in the same evidential dynamics that underwrite preemption; whether that evidence acts as noise or starts an invasion is jointly shaped by coordination equilibria and the supply conditions themselves, with drift as the finite-sampling noise in both directions. Note that biological terms like \term{niche}, \term{invasion}, and \term{preemption} are used throughout as functional analogues, not literary metaphors; following the Homeostatic Property Cluster (HPC) framework \citep{boyd1999,millikan2017}, I assume that linguistic categories are maintained by causal mechanisms formally analogous to those in biology.
...
Section~\ref{sec:distinction} introduces the constitutive-etiological distinction and the formal framework. Section~\ref{sec:preemption} develops Module~1 (opportunity-sensitive learning). Section~\ref{sec:coordination} develops Module~2 (coordination equilibria). Section~\ref{sec:integration} integrates the two modules and derives stability profiles. Section~\ref{sec:predictions} states empirical predictions and disconfirmation conditions. Section~\ref{sec:conclusion} concludes.
...
\subsection{Premises of the constitutive framework}

The etiological argument developed here presupposes the \term{constitutive framework} \citep{ReynoldsKind_inprep}. In that framework, grammatical status is not an architectural primitive but a derived property. The account rests on three specific premises which must be granted for the present argument:

\begin{enumerate}
  \item \textbf{Decomposition.} Grammatical status decomposes into three multiplicative components:
  \begin{itemize}
    \item $\mathsf{map}(u,c)$ (\term{mapping viability}): Does the form support a coherent structure-value pairing given the biophysical constraints of the processor? (Fails for \ungram{The of into slept}.)
    \item $K(u,c)$ (\term{interpretive coherence}): Do the values cohere with one another and the conditioning state? (Fails for \odd{Colourless green ideas sleep furiously}.)
    \item $C_t(u,c)$ (\term{repertoire status}): Is the form part of the community's repertoire for conditioning state $c$? (Fails for \ungram{I have 25 years} in English.)
  \end{itemize}
  The stability score is their product: $\widetilde{G}_t(u,c) = \mathsf{map} \cdot K \cdot C_t$.

  \item \textbf{Externalism about status.} The component $C_t$ is a community-level variable, not an internal mental state. It tracks the distribution of licensing in the population. While individual competence contributes to this distribution, the variable $C_t$ itself describes the specific communicative environment the learner navigates.

  \item \textbf{Continuity and Thresholding.} Status is fundamentally continuous ($\widetilde{G} \in [0,1]$) but is treated categorically in high-stakes coordination. Categoricality arises when the stability score clears a situation-specific threshold $\tau(c)$, fixed by the cost of coordination failure: $G_t = \mathbb{I}\!\left[\widetilde{G}_t \ge \tau(c)\right]$.
\end{enumerate}

These premises are taken as axioms for the remainder of this paper, which focuses exclusively on the etiology of $C_t$.

\subsection{This paper's focus: $C_t$ trajectories}

Everything above is \emph{constitutive} \citep[see][for full development]{ReynoldsKind_inprep}. It specifies what grammatical status is, not what produces it. This paper is concerned entirely with $C_t$~-- the repertoire-status component~-- and with three etiological questions about it.

\paragraph{What $C_t$ is.} $C_t(u,c)$ is a community-level variable: the probability that a randomly sampled competent community member will treat $u$ as licensed in conditioning state $c$. This is an \emph{epistemic} characterisation~-- it describes the distribution of internal grammar-states or licensing beliefs across the population. But $C_t$ is also observable \emph{behaviourally}: it manifests as the population frequency with which $u$ is produced (or accepted without repair) for a given communicative job under conditioning state $c$. The two characterisations are linked by a working assumption that production and acceptance behaviour usually track internal licensing states in aggregate. If stigma or strategic avoidance suppresses licensed forms, usage data will understate $C_t$ and the inference will need adjustment. Module~1 below provides a microfoundation for how individual beliefs about licensing collapse toward zero under preemption. Module~2 provides a microfoundation for how social responses (correction, repair, sanction) make those beliefs stable and mutually reinforcing. The two modules target different levels of analysis but converge on the same community-level outcome.
...
Here \enquote{surprisal} is shorthand for elevated prediction error, not a commitment to any specific processing architecture. All of these can implement the penalty structure that Module~2 requires, and all can feed into the avoidance behaviour that generates the opportunity patterns Module~1 exploits.
...
\term{Module~1: Opportunity-sensitive learning} (\S\ref{sec:preemption}) explains how repertoire status is inferred. The mechanism is Bayesian learning under informative opportunity sets: positive evidence entrenches a form; systematic absence where competitors win pushes status toward zero. When a form faces strong competitors that reliably win in well-attested niches, the learner accumulates strong negative evidence; $C_t$ crashes. When the opportunity set is sparse or the competitors are weak, $C_t$ drifts slowly and may never reach categorical exclusion.
...
\section{Module 1: Opportunity-sensitive learning}\label{sec:preemption}

The first module addresses Question~1: how does $C_t$ get driven toward zero (or entrenched toward one) for forms that are structurally viable and semantically coherent? The core mechanism is Bayesian updating: positive evidence ($s_t$) pushes status up; systematic absence where competitors succeed ($f_t$) pushes it down. When absence dominates, learners infer \term{statistical preemption}~-- exclusion on the basis of patterned non-occurrence \citep{goldberg2019, Goldberg2011}. But preemption strength isn't uniform across constructions. It depends critically on \emph{how much opportunity} the form has had to appear.
...
For any niche $n$ in conditioning state $c$, define the \term{competitor set} $V_{n,c}$ as the set of grammatical variants that can realize $n$ in $c$. The candidate set is supplied primarily by \term{analogical extension} from existing resources: speakers recruit available forms to realize the intention, creating new candidates where established ones are gaps. The conditioning state matters: competitors available in casual speech may differ from those available in formal writing. The age-stating niche in colloquial English has competitors including \mention{I'm 25}, \mention{I'm 25 years old}, and \mention{I'm 25 years of age}. The question niche has competitors including fronted interrogative phrases (\mention{What did you buy?}) and in-situ interrogative phrases (\mention{You bought what?}), though the latter is pragmatically restricted.
...
\paragraph{Where processing costs live.} Processing costs arise from two sources: structural properties of the form (garden-path potential, center-embedding, working memory load) and expectation-based difficulty (forms with low $C_t$ are less predicted, less primed, harder to integrate). A constitutive account has to make an explicit commitment about how these costs enter the stability score. Two options are defensible:
...
The Bayesian update in \eqref{eq:beta-update} is intrinsically bidirectional. While preemption describes how competitor tokens ($f_t$) drive the posterior toward zero, the same update defines the logic of entry: tokens of $u$ ($s_t$) push the posterior upward. Innovation is thus defined as whatever makes $s_t$ nonzero for forms that are not yet mutually expected. This is a schematic update rule: it tracks evidential effects, not a claim that learners compute Beta posteriors.

\paragraph{Supply and invasion.} Entry decomposes into two distinct phases of the positive-evidence stream \citep[cf.][on convention emergence]{skyrms2010}. \term{Supply} concerns how tokens of $u$ are generated when $C_t(u,c)$ is low: analogical extension, contact injection, performance variation, stylistic play, reanalysis. The supply rate can be structured and conditioning-state dependent~-- it is not random noise but a socially patterned injection stream that varies by niche, register, and network position. In cultural evolution terms, the supply stream folds in random variation, guided variation, and biased transmission \citep{boydricherson1985}.

\term{Invasion} concerns whether the resulting token stream becomes self-sustaining, so that $s_t$ is increasingly generated endogenously (because competent speakers now produce $u$ as a normal choice) rather than exogenously (because the innovation keeps being injected). The supply/invasion split is a bookkeeping device, not a full innovation model.
...
\paragraph{Entry is hard.} In coordination-game terms, a rare variant is typically a bad best response. The game-theoretic language is a modelling shortcut, not a claim that speakers solve explicit best-response problems. When almost nobody uses it, producing it is costly~-- misunderstanding, social penalty, processing penalty for hearers~-- so rational speakers avoid it, which keeps it rare. This is the familiar \enquote{new convention can't invade} result \citep{young1993}. Entry requires either raising the effective supply rate or lowering the coordination barrier:
...
\paragraph{Deviation penalties.} \citet{oconnor2019unfairness} emphasizes that deviation from coordination equilibria attracts penalties. The cultural evolution literature calls this \term{distributed enforcement} or, more strongly, \term{moralistic punishment} \citep{richersonboyd2005}: if a norm is established, deviants are penalized, and the penalty is enacted by whoever encounters the deviation~-- no central authority required. Conformist and prestige-biased transmission can damp deviation even without direct sanction \citep{boydricherson1985}. The label \enquote{moralistic} may overstate the affective component for grammar; the key point is that penalties are distributed and self-sustaining.
...
The framework includes a threshold $\tau(c)$: a form is in the repertoire only if $\widetilde{G}_t(u,c) \ge \tau(c)$. What determines $\tau$?
...
\toprule
 & \textbf{Module 1: Learning} & \textbf{Module 2: Coordination} \\
\midrule
Question & How is $C_t$ updated? & Why does $C_t$ stay near zero? \\
...
\subsection{Predictions from Module 1 (learning)}
...
Recent work \citep{ReynoldsKind_inprep} specifies what grammatical status \emph{is}: a stability score combining mapping viability, interpretive coherence, and repertoire status, thresholded by situation-specific coordination stakes. But constitutive accounts leave open the etiological question: why do $C_t$ trajectories settle where they do?

This paper has developed two complementary modules that answer that question. Module~1 (\emph{opportunity-sensitive learning}) explains how learners update $C_t$: Bayesian updating under informative opportunity sets allows learners to entrench forms with positive evidence and infer exclusion from systematically absent forms where competitors reliably succeed. Module~2 (\emph{coordination equilibria}) explains why $C_t$ stays near zero: once an equilibrium excludes a form, deviation is penalized, expectations crystallize, and the exclusion becomes self-sustaining. These modules interact with \emph{stochastic drift}, which can push rare forms out of the repertoire by chance, independently of systematic pressures.
...
Fourth, the framework connects grammatical structure to broader theories of convention, norm enforcement, and cultural evolution. The resources drawn upon are domain-general: Lewis on coordination, Skyrms on signaling dynamics, O'Connor on convention enforcement, Richerson and Boyd on distributed enforcement, and Young on stochastic stability \citep{lewis1969,skyrms2010,oconnor2019unfairness,richersonboyd2005,young1993}. The etiological account positions grammar within the broader ecology of socially sustained regularities, where normed systems produce categorical boundaries through ongoing dynamics.

The two-module framework is a deliberate simplification and a how-possibly model. It shows how preemption, coordination, and drift can generate stable categorical gaps, but it doesn't claim these are the only mechanisms or that every gap arose this way.

Language is a complex adaptive system~-- shaped by iterated interactions among heterogeneous agents under cognitive, social, and communicative constraints \citep{beckner2009}~-- and such systems exhibit nonlinear interactions, path-dependence, and sensitivity to initial conditions that this equilibrium-focused analysis leaves out. The modules explain \emph{stability}: why a gap, once established, persists.

Which gaps exist in a given language is historically contingent~-- the outcome of path-dependent trajectories that the modules do not predict \citep[cf.][on contingency vs.\ convergence]{powell2020}. The modules are tractable first approximations, not claims that the real dynamics decompose cleanly. Agent-based modeling may be essential, not merely complementary, for capturing emergent properties that analytic treatment obscures; \citet[ch.~16]{reynolds2025hpcbook} develops this argument in detail.

Several issues remain unaddressed beyond this fundamental simplification. First, the framework predicts stable states and monotonic transitions but doesn't explain \term{oscillating trajectories}~-- forms that rise and fall cyclically, as in fashion-driven variation or competing prestige reversals. Modeling oscillation would require incorporating time-varying payoff structures or multiple competing prestige sources.

Second, the paper treats coordination equilibria as the primary source of stability, but \term{cultural attractors}~-- stable points in form space arising from shared cognitive biases \citep[cf.][]{sperber1996}~-- may provide an independent or complementary stabilizing force. Forms with high $\mathsf{map}$ (cognitively favored) might be attractor-sustained rather than coordination-sustained, with different empirical signatures: attractor-sustained forms should be stable across social contexts, while coordination-sustained forms should be context-sensitive. Distinguishing these empirically is future work.

Third, the framework sketches the dynamics qualitatively but doesn't provide full \term{replicator-mutator formalizations} of the sort developed in evolutionary game theory \citep[e.g.,][]{nowak2006}. Such formalizations would yield sharper quantitative predictions about threshold-crossing, invasion rates, and equilibrium selection~-- but at the cost of parameter estimation challenges that may exceed current data availability.

These are defeasible causal claims. If the predicted dissociations don't appear, or if opportunity mass and coordination stakes fail to track exclusion strength, the causal story should be revised.
