...
Of course, linguistic practice already deploys multiple diacritics~-- *, ?, \#, \%~-- looking, appropriately, like the speech bubble of a comic-strip character who has stubbed their toe. But their semantics varies across authors and traditions. Some use \# for semantic anomaly, others for pragmatic infelicity; ? can mark gradient unacceptability or mere unfamiliarity. The taxonomy proposed here doesn't claim that linguists have ignored these distinctions. Rather, it attempts to give the existing diacritical toolkit a principled mapping onto underlying failure modes. The goal is to make explicit what the marks are supposed to be tracking, so that disagreements can be diagnosed rather than merely registered.
...
A detail that tends to get lost in the standard origin story is that Chomsky never treated intuitive anomaly as a simple binary. Already in \textcite{chomsky1975lslt} (written in 1955) he distinguishes different sources and \emph{degrees} of deviance, separating outright word-salad strings that resist category assignment from violations of strict subcategorisation (valence) and from violations of selectional restrictions. He returns to the point explicitly in \textcite{chomsky1961methodological}, where he isolates a family of contrasts under the heading \mention{degrees of grammaticalness} (with \S5 reprinted as \textcite[384--389]{chomsky1964degrees}). The brief reprint immediately drew philosophical replies in the same volume \autocite[390--399]{ziff1964understanding, katz1964semi}, and it was taken up independently by \textcite{cohen1965degree}.

What matters, looking back, isn't that the heterogeneity was unnoticed, but that it was largely bracketed in day-to-day syntactic theorizing: routine asterisk-practice kept functioning as a one-bit diagnosis even when the literature had explicitly acknowledged graded and structurally distinct anomaly types. A useful way to state the moral for the present paper is that this bracketing isn't merely sociological; it's partly licensed by the generative--enumerative identification of grammars with set-generating devices, within which \mention{partial grammaticality} has no natural representational home and tends to be handled via overlays rather than through a measurement model \autocite{pullum_scholz2001mtsges}.
...
Example (\ref{ex:pp-yesterday}), discussed above, illustrates: interpretive instability grounded in conventional form--value relations, rather than structural nonsense.
...
Four concepts need to be kept apart. \term{Appropriateness} is the genus: the fit between a form and the context in which it's used. \term{Grammaticality} is one species of appropriateness~-- the coupling between grammatical form and the values it conventionally expresses. It has a fact of the matter~-- but the fact in question is about community coordination, not Platonic membership. Either the form--value relation is stably treated as a legitimate resource by the norm-centred population for the relevant conditioning state, or it isn't. This is a claim about collective practice, not about abstract linguistic objects; it's realist in the sense that the coordination state is determinate even when the measurements are noisy, not in the sense that grammaticality floats free of speakers.\footnote{The distinction between ontological fact and epistemic access is developed in \textcite[ch.~5]{reynolds2025hpcbook}, which argues that category boundaries are structurally determinate but located at thresholds that cannot be finitely specified; gradient judgments arise from discrete categories filtered through processing noise, not from gradient membership.}

\term{Acceptability} is the measurement channel: how speakers rate utterances, informed by grammatical status but also by processing factors, repair costs, and ideological filtering. \term{Correctness} is the prescriptive overlay: what gatekeepers enforce, what gets codified and moralized~-- often an ideologized version of one variety's appropriateness norms imposed as if universal. Fit, status, report, enforcement: related, but not the same.

A predictable objection to the framework below is that $C_t$ (repertoire status) merely relabels prescriptive correctness. The answer is no: correctness concerns what should be enforced; repertoire status is a constitutive fact about what a norm-centred population actually treats as a legitimate resource. Enforcement can distort evidence, but the repertoire state isn't the prescription \autocite[for parallel distinctions]{pullum2019-normativity}.
...
The decomposition matters because apparent disagreement about grammaticality often reflects misalignment in $A$ or $I$ rather than genuine conflict about the state of the form--value relation. The same token can be processed as dialectal (in-repertoire under one ascription) or as an error (out-of-repertoire under a different norm-centre), depending on which conditioning anchors the listener infers. This is why the classroom/lunch contrast for \mention{I seen it} involves more than formality ($S$): whose norms are in play ($I$) and what categorization the listener assigns to the speaker ($A$) matter equally.
...
The reader now has two quantities: a binary gate ($\mathsf{map}$: can the system analyse the form at all?) and a continuous coherence measure ($K$: do the values stabilise?). The third adds community convention: even if the form parses and the values cohere, is it treated as a legitimate resource?

This third quantity is repertoire status. Let $C_t(u,c)\in[0,1]$ track whether the form--value relation $u$ is a stabilised coordinative resource in the norm-centred repertoire relevant for $c$: whether there's a stable population-level practice in which producers use $u$ for its conventional job and consumers treat it as an ordinary option (rather than as a slip, transfer artefact, or alien code) under the conditions that normally obtain for that repertoire. As an epistemic model of our access to this state, I represent uncertainty about repertoire membership by a population-level posterior probability: the probability that an individual drawn from the relevant norm-centred population treats $u$ as a legitimate resource in $c$ (where 1 represents universal acceptance and 0 indicates total exclusion). Corpus counts and task responses are evidence about $C_t$; they aren't what $C_t$ consists in. This quantity is related to what usage-based work calls entrenchment, but it's explicitly conditioned on $c$ and includes normative dimensions that pure entrenchment doesn't capture.
...
The $S$/$A$/$I$ decomposition of $c$ introduced above clarifies what fixes \enquote{the community} for a given evaluation. Identification ($I$) is the natural anchor for whose repertoire counts in $C_t$: the reference population is whoever the speaker is orienting to. Situation ($S$) and stakes are the natural anchors for the decision regime $\tau(c)$: high-stakes institutional contexts raise the threshold. Ascription ($A$) explains a major source of apparent inconsistency: the same string can be treated as in-repertoire when attributed to one ascribed group and as an error when attributed to another, even if the underlying variety grammar is the same. That reflects a change not in $\mathsf{map}$ or $K$ but in how listeners map tokens to norm-centres.
...
The three quantities (summarized in Table~\ref{tab:symbols}) now need to be combined. The intuition is simple: if any one of them fails~-- if the form can't be parsed, if the values don't cohere, or if the community doesn't treat it as a legitimate resource~-- the whole relation is ungrammatical. Partial success on one dimension can't rescue total failure on another.

Define a graded stability score:
\begin{equation}\label{eq:stability}
\widetilde{G}_t(u,c)=\mathsf{map}(u,c)\cdot K(u,c)\cdot C_t(u,c).
\end{equation}
Multiplication ensures that a zero on any component yields a zero overall. Stability also underwrites gradience: lowering any component reduces the score.
...
This formalizes an intuition that's often stated informally but rarely built into the state theory:
what counts as \enquote{grammatical} for practical purposes depends on the decision regime of
the situation, not just the resource itself. Figure~\ref{fig:minimal} summarizes the minimal
architecture. An interactive illustration is available in the project repository:\footnote{%
\url{https://github.com/BrettRey/Grammaticality-de-idealized-v2/blob/main/%
subprojects/asterisk-de-idealized/conditioned-stability-interactive.html}}
...
\begin{tikzpicture}[
  ...
  \node[observed,anchor=west] (ratings) at ($(Ct.west)+(0,-\RowSep)$) {\shortstack{feeling of\\ungrammaticality}};
  \node[observed,anchor=west] (ideo) at ($(ratings.east)+(\Dx,0)$) {\shortstack{ideological\\filtering}};
  \node[observed,anchor=west] (proc) at ($(ratings.east)+(\Dx,-\RowSep)$) {\shortstack{processing\\costs}};
  ...
  \path (proc) edge (ratings.south east);

\end{tikzpicture}
\caption{The minimal state architecture. Grammatical status $\widetilde{G}_t$ combines mapping, coherence, and repertoire status. The feeling of ungrammaticality reflects $\widetilde{G}_t$ filtered through processing costs and ideological filtering, while categorical membership $G_t$ involves a situation-specific threshold $\tau(c)$.}
\label{fig:minimal}
\end{figure}
...
In short, $K \times C_t$ manipulations should compound under the product rule; if they don't, the operator is wrong even if the decomposition stands.
...
A non-transfer L1 contrast illustrates the distinction. Consider the stigmatized double-modal construction \mention{I might could do that}, common in some Southern US varieties but categorically rejected by speakers of other varieties. The structural reason is straightforward: in Standard English, modals select plain-form VP complements, but modals are defective and lack plain forms~-- there is no plain \mention{could} to serve as complement to \mention{might}. Southern varieties have resolved this gap; Standard English hasn't. For speakers who lack the construction, $C_t\approx 0$: it isn't in their repertoire. But $K$ is high: the intended meaning (epistemic possibility of ability) is transparent, and speakers readily paraphrase it as \mention{I might be able to do that}. The prediction is that \mention{might could} under a treat-as-legitimate framing will yield high paraphrase agreement and consistent inference patterns (high $K$), supporting a low-$C_t$ rather than low-$K$ diagnosis. This is a within-L1 test of the separation, independent of transfer.
...
Take the case of a speaker who says \mention{I seen it} in a formal presentation triggers high surprisal relative to the listener's inferred $c$ (formal register, institutional norms); the same string at a barbecue triggers low surprisal. The feeling of ungrammaticality tracks the former situation, not the latter~-- even though the form's repertoire status hasn't changed.

Processing costs and ideological overlays layer on top, but the core input to the detector is \term{surprisal}, or $-\log P(u\mid c)$~-- a measure of how unexpected $u$ is given the context $c$. This framing has two methodological payoffs. First, it gives a principled bridge from ratings to a measurement channel: ratings are observations of a detector whose input includes surprisal plus processing costs, not direct observations of $C_t$. Second, it makes the later discussion of language models (\S\ref{sec:future}) less risky: LMs approximate something like $P(u\mid \text{context})$ for some training-conditioned mixture of $c$'s, which is naturally closer to \enquote{detector input} than to \enquote{truth about $G_t$}.
...
\begin{sloppypar}
Left-branch extraction in (\ref{ex:lbe}) behaves differently. The communicative niche is common, competitors are available (\mention{Which car did you buy?}), and speakers show robust categorical rejection. This profile is analyzed as near-zero repertoire status in the relevant norm-centres, consistent with a preemption-based trajectory \autocite{Goldberg2011}. In this view, categoricality needn't be located in $\mathsf{map}$: the intended analysis can be available and interpretation can be coherent once stipulated, while the community treats the relation as excluded from the repertoire (for a corpus-based implementation using Bayesian partial pooling over construction-specific opportunity sets, see \citealt{reynolds2026lbe}).
\end{sloppypar}

\subsection{Illusory ungrammaticality and misattribution}

Processing-driven illusions illustrate why the feeling of ungrammaticality can't be equated with grammatical status. Centre embedding (\ref{ex:center})'s analyzable and interpretable, but incremental parsing strains working memory and dependency integration, triggering strong negative affect \autocite{gibson2025}. Similarly, garden-path items can feel nonsensical until reanalyzed:

\ea \mention{The old man the boats.} \autocite{ritchie1984}\label{ex:old-man}
\z

A first-pass parse yields nonsense; reanalysis yields a coherent, licit structure. In such cases, ratings track repair difficulty, not repertoire status \autocite[cf.][on acceptability as an introspective report of processing cost]{gibson2025}. Conversely, illicit structures can pass unnoticed when meaning is compelling, yielding false negatives \autocite{pullum2009}.
...
\end{tikzpicture}
\caption{The medium-resolution architecture. Adds observable indicators for $C_t$ and shows ideological filtering's dual role. Solid arrows represent synchronic generative influence; the dashed arrow indicates a diachronic etiological pathway (policing reshapes $C_t$ trajectories over time). Inference runs opposite to arrow direction.}
\label{fig:medium}
\end{figure}
...
Opportunity-normalisation is the hinge between mere corpus rarity and evidence of systematic exclusion. The general problem is that niches aren't directly annotated in corpora: the variable \enquote{the speaker needed to express X} is rarely observed directly. A workable starting point is to use competitor forms as a lower-bound proxy for opportunities. If speakers reliably realise a niche using an established competitor, then each observed competitor token witnesses an opportunity in which the target variant could, in principle, have been selected.
...
What is the etiology of stable gaps? The present paper has remained mostly constitutive. A companion paper develops the etiological side through two integrated modules. The first~-- \term{opportunity-sensitive preemption}~-- models how $C_t$ gets driven toward zero: learners accumulate evidence from the structured absence of a form in niches where competitors succeed, and preemption strength scales with effective opportunity count. The second~-- \term{coordination equilibria}~-- models why $C_t$ stays near zero: once an equilibrium excludes a form, unilateral deviation is penalized, so the exclusion becomes self-sustaining. The integration yields a typology of stability profiles: gaps sustained by both modules are maximally stable; gaps sustained by only one drift under appropriate manipulations; gaps sustained by neither never fully categoricalize.
...
The coordination module draws on evolutionary game theory \autocite{oconnor2019games, lewis1969}. On this view, communicative situations are payoff structures, and repertoire boundaries stabilize because they solve recurring coordination problems. Once a contrast is salient, speakers and listeners expect each other to respect it, and deviation is costly. This explains why $C_t$ can remain near zero for forms that are structurally viable and interpretable~-- the coordination equilibrium excludes them. It also explains why certain boundaries resist erosion even under exposure: the equilibrium is self-sustaining because unilateral deviation is penalized. The threshold $\tau(c)$ fits naturally: high-stakes situations are precisely those where coordination failure is costly, and institutions often encode the expected equilibrium as explicit gatekeeping.
...
Thanks to Peter Evans, Geoff Pullum, Muhammad Ali Khalidi, Ryan Nefdt, Irene Kosmas, Mostafa Hasrati, and Henri Kauhanen for comments and suggestions on a previous version of this paper.

\medskip
\noindent\textbf{AI assistance disclosure.} The following large language models served as drafting and editing aids throughout the preparation of this paper (January 2026): Claude Opus 4.5, GPT-5.2 pro, and Gemini 3. These tools were used for prose drafting, literature search suggestions, LaTeX formatting, and editorial feedback on argumentation. I take full responsibility for all theoretical claims, arguments, analytical decisions, and interpretive choices.
