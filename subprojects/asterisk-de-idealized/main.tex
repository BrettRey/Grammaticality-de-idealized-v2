...
One central challenge involves operationalizing \term{opportunity}. Preemption-based accounts require not only token counts but niche counts: how often the communicative job arises in the relevant $c$. A key empirical task for the moving-forward agenda is to develop operational definitions of niches for different constructions and to measure non-occurrence relative to those opportunities. For one worked example, \textcite{reynolds2026lbe} defines construction-specific opportunity sets for English determiner--head packaging (e.g.\ fronted-determiner arcs in interrogatives and relatives) and estimates rates of workaround use and LBE-like discontinuity under Bayesian partial pooling; the result is a niche-relative measurement of non-occurrence that grounds preemption claims in corpus data.
