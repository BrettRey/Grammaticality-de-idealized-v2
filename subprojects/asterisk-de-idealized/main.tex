...
Chomsky's (\ref{ex:cgi}) was designed to show that structural well-formedness does not reduce to semantic plausibility. That point remains foundational: a theory that equates grammaticality with \enquote{interpretability} will misclassify many robust structural constraints. Yet (\ref{ex:cgi}) also revealed a complementary fact: humans routinely accept structurally well-formed utterances whose values are conceptually odd, while rejecting other utterances whose intended interpretation is transparent. This tension motivated a long tradition of work linking acceptability to interpretive pressures, including semantic motivation for constraints \autocite{lakoff1971, mccawley1968} and constructional meaning \autocite{goldberg1995constructions}.

What these traditions collectively established is not that meaning replaces grammar, but that the stability of interpretation is itself a locus of constraint. An utterance may be structurally viable and yet fail because the values encoded by its parts cannot be reconciled under the constraints that are live in a situation. The present perfect plus a deictic past adverbial in (\ref{ex:pp-yesterday}) is a canonical case: the intended meaning is obvious, but the morphosyntactic temporal value conflicts with the adverbial anchoring. Conversely, many lexical clashes are tolerated as long as they do not implicate morphosyntactic value.
...
A third class of cases are structurally viable and interpretively coherent, yet rejected because they are not licensed as resources in a community's repertoire. Here the role of usage and norms is constitutive: the community has not conventionalised the relevant form--value relation as a legitimate option under the norm-centres that define the communicative situation.

(\ref{ex:age}) is again emblematic. The form is not nonsensical, and it is interpretable. Its rejection is a fact about English community conventions, not about universal cognitive limits. Crucially, the same form is licit in other languages, which shows that the relevant factor is licensing, not viability or coherence.
...
This formalises an intuition that is often stated informally but rarely built into the state theory: what counts as \enquote{grammatical} for practical purposes is not only about the resource itself but about the decision regime of the situation.
...
A raw corpus absence is compatible with two very different states. A construction can be rare because the opportunity set is tiny, leaving speakers with little evidence either way; or it can be rare because, despite a large opportunity set, it is systematically preempted by competitors, driving licensing toward zero. The independent relative genitive in (\ref{ex:whose}) plausibly belongs to the first class for many speakers: the configuration that would make it useful is itself rare, so the absence of tokens does not straightforwardly imply categorical exclusion.
...
The state theory predicts that such dissociations are not exceptional; they are expected whenever the anomaly signal pools multiple sources of difficulty.

\section{Evidence and measurement: what it would take to test the state theory}

A \enquote{moving forward} programme must specify what would count as evidence. The constitutive variables suggest a principled division of labour among data types.
...
If grammaticality is conditioned, then specifying $c$ is not optional. Future work must develop operational proxies for norm-centres and communicative situations: genre, medium, stance, audience design, institutional stakes, and community membership. A crucial prediction is that constructions whose status is driven by $C_t$ will be more sensitive to $c$-manipulation than map-failures and many coherence-failures.

\subsection{What are the right objects of licensing?}

The theory treats $u$ as an utterance type, but in practice the granularity of $u$ matters. Are we licensing surface strings, abstract constructions, families of patterns? A moving-forward programme must articulate principled criteria for individuating $u$ in a way that makes the licensing term empirically meaningful rather than vacuous.
