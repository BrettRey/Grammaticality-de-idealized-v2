...
A concrete case: a speaker who says \mention{I seen it} in a formal presentation triggers high surprisal relative to the listener's inferred $c$ (formal register, institutional norms); the same string at a barbecue triggers low surprisal. The feeling of ungrammaticality tracks the former situation, not the latter~-- even though the form's repertoire status hasn't changed.

Processing costs and ideological overlays layer on top, but the core input to the detector is $-\log P(u\mid c)$. This framing has two methodological payoffs. First, it gives a principled bridge from ratings to a measurement channel: ratings are observations of a detector whose input includes surprisal plus processing costs, not direct observations of $C_t$. Second, it makes the later discussion of language models less risky: LMs approximate something like $P(u\mid \text{context})$ for some training-conditioned mixture of $c$'s, which is naturally closer to \enquote{detector input} than to \enquote{truth about $G_t$}.
