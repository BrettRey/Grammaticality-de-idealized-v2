...
Let $\mathsf{map}(u,c)\in\{0,1\}$ indicate whether there exists at least one viable morphosyntactic analysis for $u$ in $c$ for which there is a well-typed representation (where 1 is viable and 0 is not). $\mathsf{map}$ is intended to capture genuine analyzability failure and only that. It's the categorical prerequisite highlighted by the well-formedness tradition.
