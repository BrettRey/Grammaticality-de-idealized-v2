...
Consider (\ref{ex:whose}), the independent relative genitive \mention{a friend of whose}.\footnote{See \textcite{reynolds2024whose} for a detailed account of the pragmatic constraints on this construction.} The example is marked marginal in this paper's own inventory, and a central question is whether that marginality reflects low $C_t$ or merely low opportunity. A plausible niche is: `express, inside a relative clause, that an associate of the antecedent has some property'. Directly counting such niches is difficult, but we can approximate an opportunity lower bound by counting alternative realisations of the same communicative job, regardless of packaging:
(i) \mention{I saw Joan, whose friend was visiting} (dependent relative genitive);
(ii) \mention{I saw Joan, one of whose friends was visiting} (partitive relative genitive);
(iii) \mention{I saw Joan; a friend of hers was visiting} (independent non-relative genitive);
(iv) \mention{I saw Joan; one of her friends was visiting} (no genitive).
Each token of (i)--(iv) witnesses an opportunity: a speaker needed to express the relevant content and chose a competitor form.
...
This proxy operationalization is deliberately coarse, but it's already discriminating: it separates cases where \enquote{no tokens} is probative (large $N^{\ast}$) from cases where it isn't (small $N^{\ast}$). For the independent relative genitive, the prediction is clear: if $N^{\ast}$ is large and the target form is near-absent, $C_t$ is near zero~-- the form isn't in the community's active repertoire, regardless of whether it's structurally well-formed ($\mathsf{map} \cdot K > 0$). The example's marginal status is then not a brute intuition but a consequence of low $C_t$ in a well-mapped, coherent construction. More generally, this is what a \enquote{moving forward} corpus programme has to provide: explicit definitions of competitor sets for niches and principled choices of corpus slices approximating $c$.
