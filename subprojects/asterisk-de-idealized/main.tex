...
Every competent speaker of English knows that \ungram{\mention{Can the have running}} is impossible; what remains unsettled is what that impossibility consists in. When an utterance is labelled \term{ungrammatical}, are we diagnosing a failure of analysis, a failure of value, a failure of repertoire, or only a failure of comfort? The cluster in (\ref{ex:cluster}) puts the ambiguity on the page.
...
The state theory commits to just three things: analyzability, stability of construal, and repertoire status~-- each relativized to a construed situation. The notation that follows is bookkeeping, not a new philosophical burden: it's there so the asterisk can stop doing everyone's paperwork.
...
One concrete operationalisation: present a target utterance in a supporting scenario and elicit forced-choice paraphrases from $N$ participants. Compute the entropy of the paraphrase-choice distribution. High entropy (participants scatter across competing construals) diagnoses low $K$; low entropy (participants converge on a single reading) diagnoses high $K$. This distinguishes genuine instability from stable ambiguity: an ambiguous string can have high $K$ if, once a construal is selected, it stabilises without contradiction. An incoherent string doesn't stabilise under any selection.
...
Thanks to Peter Evans, Geoff Pullum, Muhammad Ali Khalidi, Ryan Nefdt, Irene Kosmas, Mostafa Hasrati, and Henri Kauhanen for comments and suggestions. The large language models Claude 3.5 \& 4.5; ChatGPT o1 pro \& 5.2 pro; Gemini 3; and DeepSeek V3 served as drafting and editing aids throughout the preparation of this paper. Their role was analogous to that of producers on a film: they generated material, proposed structures, and refined prose under my direction. I am responsible for all theoretical claims, arguments, errors, and interpretive choices.
