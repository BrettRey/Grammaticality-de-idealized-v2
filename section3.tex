...
used only when the value is recoverable from context. In \S4, a \emph{variant}
$v\in\mathcal{V}_{n,t}^{\star}$ abbreviates a candidate form--value realization
for a niche. Constructional nodes are $\kappa$, assemblies are $A$,
...
primitive. English progressive marking and Turkish evidential marking are the
worked cases there.
...
concentration is $\nu_t(\kappa,c)$ as in \S\ref{sec:update}. The individual
state $\Lambda_{i,t}$ and the population distribution over it supply the
population-level status state.
...
Two remarks keep \eqref{eq:Gtheta}--\eqref{eq:Gt} honest. First, finiteness: for a finite yield and a finite, non-empty-recursive construction inventory (no cycles of null-yield expansions--the standard offline-parsability condition), $\mathcal{A}(f,v)$ is finite, so the existential is a finite disjunction. No additional tractability stipulation is needed.
...
variant is chosen when stipulated to be available. That yields $\rho_t^\star$
...
$R_i(u,e)$ and $D_i(u,e)$ for implementation costs and prescriptive dissonance
after cue-based situation uncertainty has been integrated. The anomaly signal
accumulates low expected status, implementation costs, and ideological overlays:

\begin{align}
A_{i,t}(u,e) &= \alpha\bigl(1-\hat{G}^{(h)}_{i,t}(u\mid e)\bigr)
  +\gamma^{\top} R_i(u,e)+\delta\,D_i(u,e)+\varepsilon_i, \\
F_{i,t}(u,e) &= -\frac{\max\{A_{i,t},0\}}{1+\max\{A_{i,t},0\}}
  \in (-1,0].
\end{align}
...
registered on deviation. $D_i$ is prescriptive dissonance. The cost vector
$R_i$ includes the processing components--the locality term $L(u)$ with its
saturating form, interference, garden-path reanalysis, surprisal--and a
...
comes out grammatical and odd--the datum, with one less primitive.
